...
parser.add_argument("-p", "--path", type=str, required=True,
                    help="The path of the image upon which eyeglasses will be added")
parser.add_argument("-a", "--attrs", type=str, required=True, choices=["custom", "damiano", "giulia", "daniele", "patrizio"],
                    help="Which attributes should be used as target during generation, if 'custom' you have to modify the values inside the script before running it")
parser.add_argument("-w", "--weights", type=str, default="weights/pretrained.pth",
                    help="The path of the weights to be used during inference")
...
preprocessed_source_img = torch.unsqueeze(
    preprocessed_source_img, dim=0)   # add dummy batch size
...
generator_state_dict = {k[len("generator."):]: state_dict[k]
                        for k in state_dict.keys() if re.match('generator.*', k)}
...
target_attrs_ts = torch.unsqueeze(
    target_attrs_ts, dim=0)   # add dummy batch size
